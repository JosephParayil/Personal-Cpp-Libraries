...
static sf::Vector2f normalize_point(sf::RenderWindow& window, sf::Vector2f raw) {
...
//NOTE: Not 100% compatible. Before it was sufficient only to pass a mat4. Now you must pass the whole Camera object

void drawSphere(sf::RenderWindow& window, vec4 a, float radius, Camera& camera, sf::Color color = sf::Color::White) {
    Sphere3D sphere(a, radius);
    sphere.draw(window, camera, color);
}

void draw3DLine(sf::RenderWindow& window, vec4 a, vec4 b, Camera& camera, float thickness = 1.0f) {
...
struct Object3D_Collection {
    ...
    void depthSort(Camera& camera) {
        ...
        std::sort(c.begin(), c.end(), [&camera](const auto& a, const auto& b) {
            return a.second->getDistance(camera) > b.second->getDistance(camera);
        });
    }
};
